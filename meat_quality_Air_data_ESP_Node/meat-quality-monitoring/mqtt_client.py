...
class MQTTClient:
    ...
    def _on_disconnect(self, client, userdata, rc, properties=None):
        ...
        self.connected = False
        if rc == mqtt.MQTT_ERR_SUCCESS:
            logger.info("Disconnected from MQTT broker (requested)")
        else:
    ...
    def _process_sensor_data(self, payload: str):
        ...
        try:
            ...
            db_data = {
                ...
                'mq135_co2': sensors.get('mq135_co2', 0.0),
                'mq136_h2s': sensors.get('mq136_h2s', 0.0),
                'mq137_nh3': sensors.get('mq137_nh3', 0.0),
            ...
            # Store in database
            self.db.insert_sensor_reading(db_data)
            
            logger.info(f"Processed sensor data: T={db_data['temperature']}°C, "
                        f"H={db_data['humidity']}%, "
                        f"H2S={db_data['mq136_h2s']}ppm, "
                        f"NH3={db_data['mq137_nh3']}ppm, "
                        f"Quality={db_data['quality_level']}")
...
def reset_mqtt_client():
    """Reset the singleton MQTT client instance."""
    global _mqtt_client
