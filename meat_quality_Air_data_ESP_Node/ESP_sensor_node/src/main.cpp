 ...
 * GND  → GND (common ground)
 * SDA  → GPIO 21 (I2C SDA - default I2C pin)
 * SCL  → GPIO 22 (I2C SCL - default I2C pin)
 *
 * ESP32 NodeMCU Connections:
 * --------------------------
 * GPIO 21 (I2C SDA) → AHT10 SDA
 * GPIO 22 (I2C SCL) → AHT10 SCL
 ...
 * 3.3V              → AHT10 VCC
 * GND               → Common ground with all sensors
 * 
 * IMPORTANT NOTES:
 * ================
 ...
 *   "sensors": {
 *     "temperature": 25.5,    // Real value from AHT10
 *     "humidity": 60.0,       // Real value from AHT10
 *     "mq135_co2": 450.0,
 *     "mq136_h2s": 5.0,
 *     "mq137_nh3": 15.0
 *   },
...
Adafruit_AHTX0 aht;  // AHT10 temperature and humidity sensor
...
void setup() {
    ...
    // Initialize AHT10 temperature and humidity sensor
    Serial.println(F("Initializing AHT10 sensor..."));
    if (!aht.begin()) {
        Serial.println(F("Failed to find AHT10 sensor! Check wiring."));
        Serial.println(F("AHT10 VCC  → 3.3V"));
        Serial.println(F("AHT10 GND  → GND"));
        Serial.println(F("AHT10 SDA  → GPIO 21 (I2C SDA)"));
        Serial.println(F("AHT10 SCL  → GPIO 22 (I2C SCL)"));
    } else {
