/*
 * MQ135 Air Quality Sensor for ESP32 NodeMCU - Web Calibration Version
 * 
 * This code reads MQ135 sensor data and provides a web interface for calibration
 ...
 * --------------------------
 * GPIO 34 (ADC1_CH6) → Voltage Divider Output (MQ135 analog reading)
 * 3.3V              → Not used (MQ135 powered by 5V)
 * GND               → Common ground with MQ135
 * 
 * WEB CALIBRATION:
 * ================
 * 1. ESP32 creates SoftAP: "MQ135-Calibrator"
 ...
 * 2. MQ135 requires 5V for heater - do NOT power from 3.3V
 ...
 * 5. Typical R0 for MQ135: 10kΩ - 100kΩ (varies by sensor)
 */
...
// Load resistor value (on MQ135 module, typically 10kΩ)
const float RL = 10000.0;  // 10kΩ

// Sensor resistance in clean air (UPDATE THIS AFTER CALIBRATION!)
const float R0 = 192843.78;  // Calibrated: Average of 3 readings (191949.73, 192892.48, 193689.14)
...
const char* SOFTAP_SSID = "MQ135-Calibrator";
...
void setup() {
    ...
    Serial.println(F("\n========================================"));
    Serial.println(F("MQ135 Air Quality Sensor - ESP32 NodeMCU"));
...
void loop() {
    ...
    if (currentTime - lastReadTime >= READ_INTERVAL) {
        ...
        float nh3PPM = readMQ135();  // Using same reading for simplicity
        ...
        if (co2PPM < 600) {
            ...
            Serial.println(F("  CO2 Level: Normal"));
        } else if (co2PPM < 800) {
            Serial.println(F("  Status: GOOD"));
            Serial.println(F("  CO2 Level: Slightly elevated"));
        } else if (co2PPM < 1000) {
            Serial.println(F("  Status: MODERATE"));
            Serial.println(F("  CO2 Level: Elevated - monitor closely"));
        } else {
            Serial.println(F("  Status: SPOILED"));
            Serial.println(F("  CO2 Level: High - meat may be spoiled"));
        }
        Serial.println(F("========================================\n"));
    }
}
...
void handleSensorData() {
    ...
    float voltage = (adcValue / (float)ADC_RESOLUTION) * ESP32_VREF * VOLTAGE_DIVIDER_RATIO;
    float rs = calculateRS(voltage);
    float co2PPM = calculatePPM(rs, CO2_A, CO2_B);
...
<body>
    <div class='container'>
        <h1>MQ135 Calibration</h1>
        <p class='subtitle'>Air Quality Sensor Calibration Tool</p>
        ...
        <div class='section'>
            ...
            <div class='data-row'>
                <span class='data-label'>CO2 Concentration:</span>
                <span class='data-value' id='co2'>-- ppm</span>
            </div>
...
/**
 * Read MQ135 sensor and calculate CO2 PPM
...
float readMQ135() {
    ...
    // Calculate PPM using CO2 curve
    float ppm = calculatePPM(rs, CO2_A, CO2_B);
