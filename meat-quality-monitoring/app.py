...
import config
from mock_data import get_readings, reset_simulation, get_time_elapsed, predict_image, get_fusion_decision
from mqtt_client import get_mqtt_client, map_quality_level, determine_gas_status
...
if 'use_mock_camera' not in st.session_state:
    st.session_state.use_mock_camera = False
...
with left_col:
    ...
    with col_btn1:
        if st.button("📷 Mock Camera"):
            st.session_state.use_mock_camera = True
            st.session_state.uploaded_image = None
            st.session_state.uploaded_image_bytes = None
    ...
    if st.session_state.uploaded_image_bytes is not None:
        ...
        st.image(st.session_state.uploaded_image_bytes, width='stretch')
    elif st.session_state.use_mock_camera:
        ...
        st.info("Upload an image or use Mock Camera to start visual analysis.")
    
    # CNN Prediction
    st.markdown("### CNN Prediction Results")
    
    if st.session_state.uploaded_image_bytes is not None or st.session_state.use_mock_camera:
